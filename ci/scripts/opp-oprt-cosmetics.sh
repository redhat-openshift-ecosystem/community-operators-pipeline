...
for CSV_TO_TEST in $OPP_MODIFIED_CSVS; do
    ...
    ANSIBLE_CALLBACK_RESULT_FORMAT=yaml ansible-pull -U $OPP_ANSIBLE_PULL_REPO -C $OPP_ANSIBLE_PULL_BRANCH -i localhost, -e ansible_connection=local upstream/local.yml -e run_upstream=true --tags cosmetics -e run_bundle_test=true -e run_remove_catalog_repo=false -vv -e csv_external_file_path=$CSV_PATH_TO_TEST -e run_prepare_catalog_repo_upstream=false -e dc_stream_name=operators
done
