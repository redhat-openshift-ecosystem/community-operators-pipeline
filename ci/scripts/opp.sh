...
KIND_KUBE_VERSION=${KIND_KUBE_VERSION-"v1.32.8"}
...
OPP_OCP_PRODUCTION_VERSION_DEFAULT=${OPP_OCP_PRODUCTION_VERSION_DEFAULT-"v4.10-db"}
OPP_FORCE_DEPLOY_ON_K8S_OPENSHIFT_VERSION=${OPP_FORCE_DEPLOY_ON_K8S_OPENSHIFT_VERSION-"4.10"}
...
for t in $TESTS;do
    ...
    if [ -n "$OPP_FORCE_OPERATORS" ];then
        ...
        if [[ $t1 == orange* ]] && [[ $OPP_PROD -ge 1 ]] && [[ $OPP_CI_YAML_ONLY -eq 0 ]] && [ "$OPP_VERSION" = "sync" ];then
            ...
            # Clean CURRENT before appending (xargs trims leading/trailing spaces)
