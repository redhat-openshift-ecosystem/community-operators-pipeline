...
if [ "$1" = "openshift" ];then
  [ "$OPP_MIRROR_INDEX_MULTIARCH_BASE" != "" ] && OPP_EXEC_USER="$OPP_EXEC_USER -e mirror_multiarch_image=$OPP_MIRROR_INDEX_MULTIARCH_BASE:$OPP_MIRROR_INDEX_MULTIARCH_BASE_TAG -e mirror_apply=true -e bundle_index_image=$OPP_PRODUCTION_INDEX_IMAGE:${OPP_PRODUCTION_INDEX_IMAGE_TAG}"
